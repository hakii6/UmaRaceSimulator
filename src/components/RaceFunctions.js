...
const checkSpurt = (objParams) => {
  ...
  const spNeeded = calSpCost(v.spurting, spurtingTime, spCostCoef.spurting, raceParams)
  return sp >= spNeeded
}
  ...
  switch (umaParams.usingStyle) {
    case '1':
      switch (posKeepMode) {
        case 'normal':
          // check speedUp mode
          if (umasOrder[1].index === umaParams.index && preUma.pos - umasOrder[2].pos < 4.5) {
            if (posKeepCD !== 0) {
        ...
        case 'speedUp':
          // check end
          if (preUma.pos - posKeepStart >= sectionDist || (umasOrder[1].index === umaParams.index && preUma.pos - umasOrder[2].pos > 4.5)) {
            momentUma.otherCond.posKeepMode = 'normal'
          }
          break
        case 'overtake':
          // check end
          if (preUma.pos - posKeepStart >= sectionDist || (umasOrder[1].index === umaParams.index && preUma.pos - umasOrder[2].pos > 10.0)) {
            momentUma.otherCond.posKeepMode = 'normal'
          }
          break
      }
...
const calTargetV = (objParams) => {
  ...
  return (umaParams.v.[momentUma.moveState] * posKeepEffect + slopeEffect).round()
  ...
  switch (preUma.costState) {
    case 'tempt':
      ...
      if (momentUma.otherCond.temptCount >= 12 * framesPerSec) {
        return 'normal'
      } else if (momentUma.otherCond.temptCount >= momentUma.otherCond.temptLast) {
      ...
      break
    case 'normal':
      if (preUma.section === umaParams.temptSection && !momentUma.otherCond.ifTempt) {
        momentUma.otherCond.temptLast = 3 * framesPerSec
...
export const moveProcess = (objParams) => {
  ...
  const spCost = calSpCost(avgV, frameLength, spCostCoef.[costState], raceParams)
